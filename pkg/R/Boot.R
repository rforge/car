...
# 2017-07-22: added a vcov.boot method that simply returns cov(object$t)
# 2017-07-22: fixed args to hist.boot as suggested by Achim Zeileis
# 2017-07-22: Fixed bugs in Boot.default; updated .Rd file as suggested by Achim Zeileis

Boot <- function(object, f=coef, labels, R=999, method){UseMethod("Boot")}
...
#  b <- boot::boot(data.frame(update(object, model=TRUE)$model), boot.f, R, .fn=f)
  b <- boot::boot(data.frame(object$model), boot.f, R, .fn=f)
  colnames(b$t) <- labels
