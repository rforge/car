...
scatterplot1.formula <- function (x, data, subset, xlab, ylab, legend.title, labels, ...) {
	...
	if (ncol(X) == 2) scatterplot1(X[,2], X[,1], xlab=xlab, ylab=ylab, 
			labels=labels, ...)
	else {
		if (missing(legend.title)) legend.title <- names[3]
		scatterplot1(X[,2], X[,1], groups=X[,3], xlab=xlab, ylab=ylab,  
			legend.title=legend.title, labels=labels, ...)
	}
}

scatterplot1.default <- function(x, y, smooth=TRUE, spread=!by.groups, span=.5, loess.threshold=5, reg.line=lm, 
