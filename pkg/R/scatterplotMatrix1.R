...
scatterplotMatrix1 <- function(x, ...){
	UseMethod("scatterplotMatrix1")
}

scatterplotMatrix1.formula <- function (x, data=NULL, subset, labels, ...) {
	...
	if (!groups) scatterplotMatrix1(X, labels=labels, ...)
	else{
		ncol<-ncol(X)
		scatterplotMatrix1.default(X[, -ncol], groups=X[, ncol], labels=labels, ...)
	}
}

scatterplotMatrix1.default <- function(x, var.labels=colnames(x), 
	...
	labels, id.method = "mahal", id.n = 0, id.cex = 1, id.col = NULL,
