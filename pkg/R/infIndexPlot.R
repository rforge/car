...
infIndexPlot.lm <- function(model, 
     Cook=TRUE, Studentized=TRUE, Bonf=TRUE, hat=TRUE, main="Diagnostic Plots",
...
# check for row.names, and use them if they are numeric.
   what <- c(Cook, Studentized, Bonf, hat)
   if(missing(labels)) labels <-  row.names(model$model)
   op <- par(mfrow=c(sum(what),1),mar=c(1,4,0,2)+.0,mgp=c(2,1,0), oma=c(6,0,6,0))
